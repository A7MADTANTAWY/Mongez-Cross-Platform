...
# ── Paths ────────────────────────────────────────────────────────────────────
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path   # deployment/scripts
$DeployDir = Split-Path -Parent $ScriptDir                     # deployment
$RootDir   = Split-Path -Parent $DeployDir                     # repo root
Set-Location -Path $RootDir

$ComposeDir         = Join-Path $DeployDir "docker"
$ComposeFile        = Join-Path $ComposeDir "docker-compose.yml"
$ComposeProjectName = "mongez"
...
$DashboardEnvFile    = Join-Path $RootDir "$DashboardDir\.env"
$DashboardEnvExample = Join-Path $RootDir "$DashboardDir\.env.example"
$DashboardPidFile    = Join-Path $ScriptDir ".dashboard.pid"
$DashboardLogFile    = Join-Path $ScriptDir ".dashboard.log"
...
if ($Down) {
    ...
    Stop-Dashboard
    docker compose -f $ComposeFile -p $ComposeProjectName down -v
...
if ($Rebuild -or -not $imagePresent) {
    ...
    if ($Rebuild) {
        docker compose -f $ComposeFile -p $ComposeProjectName build --no-cache $ComposeService
    } else {
        docker compose -f $ComposeFile -p $ComposeProjectName build $ComposeService
    }
...
docker compose -f $ComposeFile -p $ComposeProjectName up -d $ComposeService | Out-Null
...
while ($true) {
    ...
    if ($status -eq "unhealthy") {
        ...
        docker compose -f $ComposeFile -p $ComposeProjectName logs --tail=50 $ComposeService
        exit 1
    }
...
'@
    $seedScript | docker compose -f $ComposeFile -p $ComposeProjectName exec -T $ComposeService python manage.py shell
...
Write-Host "  • Backend logs:  docker compose -f `$ComposeFile logs -f web"
if ($DashboardRunning) {
...
if ($FollowLogs) {
    ...
    docker compose -f $ComposeFile -p $ComposeProjectName logs -f $ComposeService
}
