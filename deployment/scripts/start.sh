...
# ── Paths ────────────────────────────────────────────────────────────────────
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"   # deployment/scripts
DEPLOY_DIR="$(dirname "$SCRIPT_DIR")"                        # deployment
ROOT_DIR="$(dirname "$DEPLOY_DIR")"                          # repo root
cd "$ROOT_DIR"

COMPOSE_DIR="$DEPLOY_DIR/docker"
COMPOSE_FILE="$COMPOSE_DIR/docker-compose.yml"
COMPOSE_PROJECT_NAME="mongez"
...
DASHBOARD_ENV_FILE="$ROOT_DIR/$DASHBOARD_DIR/.env"
DASHBOARD_ENV_EXAMPLE="$ROOT_DIR/$DASHBOARD_DIR/.env.example"
DASHBOARD_PID_FILE="$SCRIPT_DIR/.dashboard.pid"
DASHBOARD_LOG_FILE="$SCRIPT_DIR/.dashboard.log"
DASHBOARD_URL="http://localhost:5173/"

# docker compose wrapper (specify the compose file + project name explicitly)
compose() {
...
if [ "$DO_SEED" = 1 ]; then
  ...
  compose exec -T "$COMPOSE_SERVICE" python manage.py shell <<'PY'
...
if [ "$DO_LOGS" = 1 ]; then
  ...
  exec compose logs -f "$COMPOSE_SERVICE"
fi
