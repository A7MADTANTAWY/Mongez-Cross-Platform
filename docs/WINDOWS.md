...
deployment/scripts/start.ps1 -All

# Common combinations:
deployment/scripts/start.ps1 -Seed -Dashboard          # backend + dashboard, seeded
deployment/scripts/start.ps1 -Rebuild -Seed -Dashboard # rebuild image, seed, run dashboard
deployment/scripts/start.ps1 -Stop                     # stop backend + dashboard
deployment/scripts/start.ps1 -Down                     # stop and DELETE database/uploads
deployment/scripts/start.ps1 -Status                   # show docker compose -f deployment/docker/docker-compose.yml -p mongez ps
deployment/scripts/start.ps1 -FollowLogs               # tail backend logs
```
...
Windows often has process-level reservations. If `docker compose -f deployment/docker/docker-compose.yml -p mongez up`
...
If `docker compose -f deployment/docker/docker-compose.yml -p mongez build` or `npm install` is glacial, add exclusions:
...
docker compose -f deployment/docker/docker-compose.yml -p mongez logs -f web
docker compose -f deployment/docker/docker-compose.yml -p mongez down            # stop
docker compose -f deployment/docker/docker-compose.yml -p mongez down -v         # stop + wipe (DESTRUCTIVE)
```
...
re-run `deployment/scripts/start.ps1 -Seed`.
...
| Surface | How |
|---|---|
| Backend container | `docker compose -f deployment/docker/docker-compose.yml -f deployment/docker/docker-compose.override.yml build` → push the resulting image to a registry, deploy on any Linux host. |
| Dashboard         | `cd frontend; npm run build` → `firebase deploy --only hosting` (the repo ships `firebase.json`). Or zip `frontend\dist` and serve from any static host (S3, Nginx, GitHub Pages, etc.). |
...
|---|---|
| `deployment/scripts/start.sh`                    | `deployment/scripts/start.ps1` |
| `deployment/scripts/start.sh --all`              | `deployment/scripts/start.ps1 -All` |
| `deployment/scripts/start.sh --seed --dashboard` | `deployment/scripts/start.ps1 -Seed -Dashboard` |
| `deployment/scripts/start.sh --stop`             | `deployment/scripts/start.ps1 -Stop` |
| `deployment/scripts/start.sh --down`             | `deployment/scripts/start.ps1 -Down` |
...
| Dashboard loads but `/api` requests 404 | Backend isn't up — `deployment/scripts/start.ps1 -Status`. |
...
```powershell
docker compose -f deployment/docker/docker-compose.yml -p mongez exec web cp /app/data/db.sqlite3 /app/media/backup-db.sqlite3
