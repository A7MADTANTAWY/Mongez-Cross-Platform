...
| 2 | Dashboard (React + Vite SPA — landing + admin) | `frontend/` | <http://localhost:5173/> |
...
That's it. Everything else is `deployment/scripts/start.sh`.
...
```bash
deployment/scripts/start.sh --rebuild --seed --dashboard
```
...
4. Runs `npm install` in `frontend/` (idempotent — instant after the first time).
...
```bash
deployment/scripts/start.sh --down
```
...
# or, raw Docker:
docker compose -f deployment/docker/docker-compose.yml -p mongez up -d --build web
docker compose -f deployment/docker/docker-compose.yml -p mongez logs -f web        # tail logs
```
...
| Command | Effect |
|---|---|
| `deployment/scripts/start.sh` | Backend only, reuse cached image |
| `deployment/scripts/start.sh --rebuild` | Force `docker build --no-cache` |
| `deployment/scripts/start.sh --seed` | Backend + seed 3 test accounts |
| `deployment/scripts/start.sh --dashboard` | Backend + dashboard dev server (background) |
| `deployment/scripts/start.sh --mobile` | Backend + `flutter run` (foreground, blocks) |
| `deployment/scripts/start.sh --all` | `= --seed --dashboard --mobile` (full local stack) |
| `deployment/scripts/start.sh --logs` | After start, tail backend logs |
| `deployment/scripts/start.sh --status` | `docker compose -f deployment/docker/docker-compose.yml -p mongez ps` and exit |
| `deployment/scripts/start.sh --stop` | Stop backend **and** dashboard |
| `deployment/scripts/start.sh --down` | Stop **and wipe** database/uploads |

Combine freely: `deployment/scripts/start.sh --rebuild --seed --dashboard --logs`.
...
Run this block after `deployment/scripts/start.sh --seed --dashboard`. Everything should
...
```bash
deployment/scripts/start.sh --down               # stops + wipes volumes (asks for "yes")
deployment/scripts/start.sh --rebuild --seed --dashboard
```
...
deployment/scripts/start.sh --rebuild
...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez logs -f web       # backend
...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez exec web python manage.py shell
```

### Run tests (matches CI)

```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps        # backend (48 tests)
...
4. `deployment/scripts/start.sh --rebuild` (so the new ALLOWED_HOSTS takes effect).
...
| Symptom | Fix |
|---|---|
| `Ports are not available: bind: address already in use` | `deployment/scripts/start.sh --stop`, then `lsof -i :8000` (or `:5173`) and kill the holder. |
...
| Container stuck `health=starting` | `docker compose -f deployment/docker/docker-compose.yml -p mongez logs web --tail=80` — usually a migration error. |
| Migration says "no changes" but you edited a model | You're editing under `backend/apps/X/models.py`; `makemigrations` looks at `apps.X` — already correct in this repo, so just re-run `deployment/scripts/start.sh --rebuild`. |
