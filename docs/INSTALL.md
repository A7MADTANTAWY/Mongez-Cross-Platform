...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez ps
...
```bash
deployment/scripts/start.sh --seed
...
```bash
# Stop the backend
docker compose -f deployment/docker/docker-compose.yml -p mongez down

# Start again (no rebuild needed)
docker compose -f deployment/docker/docker-compose.yml -p mongez up
...
docker compose -f deployment/docker/docker-compose.yml -p mongez down -v
...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez down -v && docker compose -f deployment/docker/docker-compose.yml -p mongez up
```

**Mobile shows "connection refused"**
- Check the backend is running: `docker compose -f deployment/docker/docker-compose.yml -p mongez ps`
...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez logs --tail=100 web
...
| Task | Command |
|---|---|
| Build image | `docker compose -f deployment/docker/docker-compose.yml -p mongez build` |
| Start (background) | `docker compose -f deployment/docker/docker-compose.yml -p mongez up -d` |
| View logs | `docker compose -f deployment/docker/docker-compose.yml -p mongez logs -f web` |
| Run tests | `docker compose -f deployment/docker/docker-compose.yml -p mongez exec web python manage.py test apps` |
| Open Django shell | `docker compose -f deployment/docker/docker-compose.yml -p mongez exec web python manage.py shell` |
| Seed test accounts | `deployment/scripts/start.sh --seed` |
| Apply new migrations | `docker compose -f deployment/docker/docker-compose.yml -p mongez exec web python manage.py migrate` (also runs on boot) |
| Restart only | `docker compose restart web` |
| Stop everything | `docker compose -f deployment/docker/docker-compose.yml -p mongez down` |
| Wipe data + restart | `docker compose -f deployment/docker/docker-compose.yml -p mongez down -v && docker compose -f deployment/docker/docker-compose.yml -p mongez up -d` |
