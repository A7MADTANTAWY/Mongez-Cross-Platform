...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py check
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py makemigrations --dry-run
```
...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps --noinput
```
...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps.users
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps.workers
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps.orders
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps.ratings
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps.favorites
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps.notifications
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps.payments
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps.admin_api
```
...
```bash
docker compose -f deployment/docker/docker-compose.yml -p mongez logs -f web              # backend access + error log
```

---

## 3. Dashboard — React + Vite (`frontend/`)
...
**dashboard ↔ mobile** loop end-to-end against `deployment/scripts/start.sh --seed`.
...
| `Ports are not available: bind :8000` | Old container or another process holds the port | `deployment/scripts/start.sh --stop`, then `lsof -i :8000` (Linux) / `netstat -ano \| findstr :8000` (Windows) |
...
| Container stuck `health=starting` | Migration crashed mid-boot | `docker compose -f deployment/docker/docker-compose.yml -p mongez logs web --tail=80`, look for the traceback |
| Tests pass but `curl /api/health/` 502 | Gunicorn workers crashed | `docker compose -f deployment/docker/docker-compose.yml -p mongez logs web` — usually a config / import error introduced since last build |
...
# automated
docker compose -f deployment/docker/docker-compose.yml -p mongez exec -T web python manage.py test apps        # backend
( cd front  && npm run lint && npm run build )                # dashboard
( cd mobile && flutter analyze && flutter test )              # mobile

# clean up
deployment/scripts/start.sh --stop                                             # stop, keep data
deployment/scripts/start.sh --down                                             # stop, WIPE data
```
