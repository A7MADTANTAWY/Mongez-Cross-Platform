...
class WorkerLocationServesDefaultAddressTests(APITestCase):
    """Clients must see the worker's DEFAULT saved address, not a stale
    User-column snapshot (which used to hold the first address)."""
    ...
    def setUpTestData(cls):
        ...
        cls.worker = User.objects.create_user(
            username="stale_loc_worker",
            phone="+201000000099",
            password="Sup3r-Secret!",
            role=User.Role.WORKER,
            profile_completed=True,
            # Stale snapshot: values of the FIRST address he ever added.
            governorate="cairo",
            city="Nasr City",
            address="12 Old Cairo St",
        )
        WorkerProfile.objects.create(
            user=cls.worker, profession="Plumbing", experience_years=3,
        )
        # First added — NOT the default.
        Address.objects.create(
            user=cls.worker, label="Old flat",
            address="12 Old Cairo St",
            governorate="cairo", city="Nasr City",
            is_default=False,
        )
        # The one he later marked as default.
        cls.default_addr = Address.objects.create(
            user=cls.worker, label="New workshop",
            address="5 Maadi St",
            governorate="giza", city="Maadi",
            is_default=True,
        )

    def _assert_location_is_default(self, user_data):
        self.assertEqual(user_data["governorate"], "giza")
        self.assertEqual(user_data["city"], "Maadi")
        self.assertEqual(user_data["address"], "5 Maadi St")

    def test_worker_detail_shows_default_address(self):
        resp = self.client.get(
            reverse("worker-detail", args=[self.worker.worker_profile.id]),
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self._assert_location_is_default(resp.data["user"])

    def test_workers_list_shows_default_address(self):
        resp = self.client.get(reverse("worker-list"))
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        match = [w for w in resp.data["results"]
                 if w["user"]["username"] == "stale_loc_worker"]
        self.assertTrue(match)
        self._assert_location_is_default(match[0]["user"])
