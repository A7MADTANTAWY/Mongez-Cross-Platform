from django.db.models import Avg, Count, F, FloatField, IntegerField, OuterRef, Q, Subquery
...
from config.permissions import IsAdmin, IsWorker, IsProfileCompleted
...
class WorkerListView(APIView):
    ...
    def get(self, request):
        ...
        # Annotate order counts so SerializerMethodField avoids N+1 queries
        orders_subq = Order.objects.filter(worker=OuterRef("user"))
        queryset = queryset.annotate(
            _total_orders=Coalesce(Subquery(
                orders_subq.values("worker").annotate(c=Count("id")).values("c"),
                output_field=IntegerField(),
            ), 0),
            _completed_orders=Coalesce(Subquery(
                orders_subq.filter(status=Order.COMPLETED)
                    .values("worker").annotate(c=Count("id")).values("c"),
                output_field=IntegerField(),
            ), 0),
            _accepted_orders=Coalesce(Subquery(
                orders_subq.filter(status__in=[Order.ACCEPTED, Order.COMPLETED])
                    .values("worker").annotate(c=Count("id")).values("c"),
                output_field=IntegerField(),
            ), 0),
        )
    ...
    def get(self, request):
        ...
        recent_ratings = list(
            ...
            .values(
                ...
                "client__username", "client__name_ar",
            )[:5]
        )
        for r in recent_ratings:
            r["created_at"] = r["created_at"].isoformat()
            username = r.pop("client__username")
            # Display the client's real name; username (e.g. "google_<sub>")
            # is only a last-resort fallback.
            r["client_name"] = r.pop("client__name_ar") or username
            r["client_username"] = username
    ...
    def get(self, request, pk):
        try:
            worker = WorkerProfile.objects.select_related(
                "user"
            ).prefetch_related("user__addresses").get(pk=pk)
        except WorkerProfile.DoesNotExist:
            return Response(
